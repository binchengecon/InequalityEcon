...
def loss(model, a_interior, z_interior, a_NBC, z_NBC, a_SC_upper, z_SC_upper, a_SC_lower, z_SC_lower):
    ...
    L1_KFE = -KFE_a - KFE_z + 1/2*KFE_zz1


    L2_KFE = tf.reduce_sum(g) - 1

    L3_KFE = tf.reduce_sum(g*a_interior) 
...
for i in range(sampling_stages):
    ...
    for _ in range(steps_per_sample):
        loss,L1,L2,L3,LKFE,_ = sess.run([loss_tnsr, L1_tnsr, L2_tnsr, L3_tnsr, optimizer],
